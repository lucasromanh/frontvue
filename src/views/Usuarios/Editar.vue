<template>
<div class="editar" :style="backgroundStyle"> 
  ...
  export default {
    ...
    methods: {
      getUsuariosData(usuariosId) {
        axios.get(`http://127.0.0.1:3000/usuarios/${usuariosId}`).then(res => {
          this.model.usuarios = res.data;
        });
      },
      actualizarUsuario() {
        const usuariosId = this.$route.params.id;  
        axios.put(`http://127.0.0.1:3000/usuarios/${usuariosId}`, this.model.usuarios)
          .then(res => {
